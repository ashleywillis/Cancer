...
Tukey's HSD: Invasion
```{r}
# Malignant Transformation
tukey.pimtanova1 <-TukeyHSD(pimtanova1)
tukey.pimtanova1 
    #Nothing statistically significant...

t.pimtanova1 <- aov(mal.trans ~ invade:digit, data = placentacancer1)

tplot.pimtanova1 <- TukeyHSD(t.pimtanova1)
plot(tplot.pimtanova1, las= 1)




#Malignancy Prevalence
tukey.pimpanova <- TukeyHSD(pimpanova)
tukey.pimpanova
    #Statistically significant difference between epi & endo only

t.pimpanova <- aov(prev.mal ~ invade:digit, data = placentacancer1)

tplot.pimpanova <- TukeyHSD(t.pimpanova)
plot(tplot.pimpanova, las= 1)




#Neoplasia Prevalence
tukey.pnpanova <- TukeyHSD(pnpanova)
tukey.pnpanova
    #Nothing statistically significant
t.pnpanova <- aov(prev.neo ~ invade, data = placentacancer1)

tplot.pnpanova <- TukeyHSD(t.pnpanova)
plot(tplot.pnpanova, las= 1)

plot(tukey.pnpanova)

```

Tukey's HSD: Interdigitation
```{r}
# Malignant Transformation
tukey.imtanova <-TukeyHSD(imtanova)
tukey.imtanova
    # Statistical significance between Lab & Trab and Lab & Vill
plot(tukey.imtanova)


#Malignancy Prevalence
tukey.impanova <- TukeyHSD(impanova)
tukey.impanova
    #Statistically significant difference between (large) Lab & Vill and Lab & Trab
plot(tukey.impanova)

#Neoplasia Prevalence
tukey.inpanova <- TukeyHSD(inpanova)
tukey.inpanova
    #Nothing statistically significant
plot(tukey.inpanova)
```

Invasion: Create New Data Frames with Groupwise Differences
```{r}
# Malignant Transformation
mean.pimtanova1 <- placentacancer1 %>%
  group_by(invade, digit) %>%
  summarise(
    mal.trans= mean(mal.trans)
  )
...
combo2.4 %>%
  ggplot(.) +
  geom_boxplot(aes(x= placenta_invasiveness.x, y= birthpotent))

# Maximum offspring output x neoplasia prevalence
combo2.4 %>%
  ggplot(.)+
  geom_point(aes(x= birthpotent, y= prev.neo.x))

# Maximum offspring output x malignancy prevalence
combo2.4 %>%
  ggplot(.)+
  geom_point(aes(x= birthpotent, y= prev.mal.x))

# Maximum offspring output x malignant transformation rate
combo2.4 %>%
  ggplot(.)+
  geom_point(aes(x= birthpotent, y= mal.trans.x))
```
